...
def displacement(moving_image, target_image, mask=None):
    ...
    flow = opt_flow.calc(util.img_as_ubyte(target_image), util.img_as_ubyte(moving_image), None)
    if mask is not None:
        dx = flow[..., 0][mask != 0]
        dy = flow[..., 1][mask != 0]
    else:
...
def solve_abc(verts):
    ...
    """
    a = np.array([[verts[0, 0], verts[0, 1], 1], [verts[1, 0], verts[1, 1], 1], [verts[2, 0], verts[2, 1], 1]])
    b = verts[:, 2]
...
def get_verts(img, x, y, pos=0):
    ...
    if pos == 0:
        # Lower left
        verts = np.array([[x, y, img[y, x]], [x + 1, y, img[y, x + 1]], [x, y + 1, img[y + 1, x]]])  # BL  # BR  # TL
    if pos == 1:
        # Upper right
        verts = np.array(
            [[x, y + 1, img[y + 1, x]], [x + 1, y, img[y, x + 1]], [x + 1, y + 1, img[y + 1, x + 1]]]  # BL  # BR  # TL
        )
...
@nba.njit()
def update_joint_H(binned_moving, binned_fixed, H, M, sample_pts, pos=0, precalcd_abc=None):
    ...
    for i, sxy in enumerate(sample_pts):
        ...
        for alpha1 in range(0, q):
            for alpha2 in range(0, q):
                ...
                if xy[0] <= x_lims[0] or xy[0] >= x_lims[1] or xy[1] <= y_lims[0] or xy[1] >= y_lims[1]:
                    continue

                    #  Determine if intersection inside triangle ###
                vote = isInside(
                    img1_v[0, 0], img1_v[0, 1], img1_v[1, 0], img1_v[1, 1], img1_v[2, 0], img1_v[2, 1], xy[0], xy[1]
                )
...
class AffineOptimizer(object):
    ...
    def setup(self, moving, fixed, mask, initial_M=None):
        ...
        if initial_M is not None:
            (tx, ty), rotation, (scale_x, scale_y), shear = get_affine_transformation_params(initial_M)
    ...
    def calc_cost(self, p):
        ...
        return self.cost_fxn(
            self.pyramid_fixed[self.current_level], transformed, self.pyramid_mask[self.current_level]
        )
    ...
    def align(self, moving, fixed, mask, initial_M=None, moving_xy=None, fixed_xy=None):
        ...
        for n in levels:
            ...
            if other_params is None:
                ...
                param_bounds = [[0, np.deg2rad(360)], [-max_tc, max_tc], [-max_tr, max_tr]]
...
class AffineOptimizerMattesMI(AffineOptimizer):
    ...
    accepts_xy : bool
        Whether to use the matching features between the images pair as landmarks
        for the registration or not.
    ...
    def align(self, moving, fixed, mask, initial_M=None, moving_xy=None, fixed_xy=None):
        ...
        M = transform.SimilarityTransform(scale=scale, rotation=rotation, translation=(tx, ty)).params
        ...
        if moving_xy is not None and fixed_xy is not None:
            ...
            tform_files = [f for f in os.listdir(".") if f.startswith("TransformParameters.") and f.endswith(".txt")]
...
class AffineOptimizerOffGrid(AffineOptimizer):
    ...
    def setup(self, moving, fixed, mask, initial_M=None):
        ...
        self.z_range = (
            min(np.min(self.moving[self.nlevels - 1]), np.min(self.fixed[self.nlevels - 1])),
            max(np.max(self.moving[self.nlevels - 1]), np.max(self.fixed[self.nlevels - 1])),
        )

        self.grid_spacings = [
            self.get_scpaing_for_levels(self.pyramid_fixed[i], self.spacing) for i in range(self.nlevels)
        ]
    ...
    def get_scpaing_for_levels(self, img_shape, max_level_spacing):
        ...
        shape_ratio = np.mean([img_shape[0] / max_shape[0], img_shape[0] / max_shape[0]])
    ...
    def get_interp(self, img):
        return interpolate.RectBivariateSpline(
            np.arange(0, img.shape[0], dtype=np.float), np.arange(0, img.shape[1], dtype=np.float), img
        )
    ...
    def calc_cost(self, p):
        ...
        if (
            ...
            or np.any(warped_corners[:, 0] > self.pyramid_fixed[self.current_level].shape[0])
            or np.any(warped_corners[:, 1] > self.pyramid_fixed[self.current_level].shape[1])
        ):
            return np.inf
        ...
        fixed_intensities = self.interp_point(
            warped_xy[:, 1], warped_xy[:, 0], self.fixed_interps[self.current_level], self.z_range
        )
